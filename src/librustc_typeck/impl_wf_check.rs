...
use crate::constrained_generic_params as cp;
use rustc::hir;
                                                 ...
                                                 impl_item_refs: &[hir::ImplItemRef])
{
    ...
    let mut input_parameters = cp::parameters_for_impl(impl_self_ty, impl_trait_ref);
    cp::identify_constrained_generic_params(
    ...
    let lifetimes_in_associated_types: FxHashSet<_> = impl_item_refs.iter()
        ...
        .flat_map(|def_id| {
            cp::parameters_for(&tcx.type_of(def_id), true)
        }).collect();
    ...
    for param in &impl_generics.params {
        match param.kind {
            ...
            ty::GenericParamDefKind::Type { .. } => {
                ...
                if !input_parameters.contains(&cp::Parameter::from(param_ty)) {
            ...
            ty::GenericParamDefKind::Lifetime => {
                let param_lt = cp::Parameter::from(param.to_early_bound_region_data());
            ...
            ty::GenericParamDefKind::Const => {
                ...
                if !input_parameters.contains(&cp::Parameter::from(param_ct)) {
                    report_unused_parameter(tcx,
                                           tcx.def_span(param.def_id),
                                           "const",
                                           &param_ct.to_string());
