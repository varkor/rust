...
impl Region {
    ...
    fn late(hir_map: &Map, param: &hir::GenericParam) -> (hir::LifetimeName, Region) {
        ...
        let (name, def_id, origin) = new_region(hir_map, param);
        debug!("Region::late: depth={:?} def_id={:?}", depth, def_id);
...
impl<'a, 'tcx> LifetimeContext<'a, 'tcx> {
    ...
    fn resolve_lifetime_ref(&mut self, lifetime_ref: hir::LifetimeRef<'tcx>) {
        ...
        if let Some(mut def) = result {
            ...
            ).span_label(lifetime_ref.span, "undeclared lifetime").emit();
        }
    }
    ...
    fn resolve_elided_lifetimes(&mut self,
        ...
        let mut err =
            report_missing_lifetime_specifiers(self.tcx.sess, span, lifetime_refs.len());
